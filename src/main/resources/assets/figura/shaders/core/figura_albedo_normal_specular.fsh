...
void main() {
    ...
    if (emissivity < 1.0) {
        vec4 normalSample = texture(Normal, normalUV); // Sample the normal texture
        ...
        color = mix(minecraft_mix_light(normalize(light0), normalize(light1), normal, color), color, emissivity);
    }
    // Apply overlay color
